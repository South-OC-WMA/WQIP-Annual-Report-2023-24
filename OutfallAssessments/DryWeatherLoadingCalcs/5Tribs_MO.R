...
Trib$FACILITYID<-sub("I01-11216-4 (I02P14)", "I01-11216-4 (I02P14)",Trib$FACILITYID)
...
MO <- paste0(wd, '/MajorOutfalls_2024.csv') 
...
#%>%
#filter(JURISDICTI != c(IRVINE, 'NEWPORT BEACH'))
...
file_url<-paste0(inPath, "JURISinTRIBS.xlsx")
JURISinTRIBS<-read_excel(file_url) 
...
JURISinTRIBS$FACILITYID<-sub("I01-11216-4", "I01-11216-4 (I02P14)",JURISinTRIBS$FACILITYID)
...
JURISinTRIBS2 <- JURISinTRIBS_MO %>% 
 ...
 # mutate(JURISDICTI3 = JURISDICTI2) %>%
  mutate(JURISDICTI3 = JURISDICTI) %>%
  mutate(JURISDICTI3=coalesce(JURISDICTI3, Jurisdiction)) %>%
  #convert jurisdiction to all capital letters 
  mutate(Jurisdiction = toupper(Jurisdiction)) %>%
...
%>%  #outfall nearest receiving water RUN LATER
...
#JURISinTRIBS2b <-JURISinTRIBS2b %>%
 ...
 # mutate(AcresOb= ((acres_juris_soc)/sumNA), AREA)) %>%
  #group_by(JURISDICTI3) %>%
  #mutate(PERCENT_o =(100*(Acres/acres_juris_soc)))
  

#saveRDS(JURISinTRIBS2b, file = paste0(outPath, 'JURISinTRIBS2b.rds'))
#write_csv(JURISinTRIBS2b, path = paste0(outPath, 'JURISinTRIBS2b.csv'))
