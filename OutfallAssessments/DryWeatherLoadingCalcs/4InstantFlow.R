...
Outfall_Inspect_ <- read_csv(paste0(wd, '/JurisdictionalFollowUps/Output/Outfall_Inspect_rawdata_flow01252025.csv'))
...
#Already done in Jurisdictional follow-ups
...
Outfall_Inspect<-read_excel(url_Outfall_Inspect) %>%
  ...
  filter(ObsRecordDate > "2023-09-30" & ObsRecordDate < "2024-10-02")
...
#list of major outfalls with instantenous flow measurement for 2024
...
#list of major outfalls with no instantenous flow measurement for 2024
MO<-read_csv(paste0(wd, '/DryWeatherLoadingCalcs/Input/MajorOutfalls_2024.csv')) %>%
...
#join with major outfalls and instant flow measurements for 2024

MO <- read_csv(paste0(wd, '/MajorOutfalls_2024.csv')) %>%
  select('Facility Identifier', 'AVGDISCHARGE', 'Jurisdiction')

MO$'Facility Identifier'[MO$'Facility Identifier' == "L04-136-1u (L04P07)"] <- "L04-136-1 (L04P07)"

MOallinsQ <- right_join(swVolumetric, MO, by=c('StationCode' = 'Facility Identifier', 'Jurisdiction' = 'Jurisdiction')) %>%
...
allInst2 <- left_join(MO, allInst, by=c('Facility Identifier'= 'StationCode', "Jurisdiction", 'AVGDISCHARGE')) %>%

mutate(Jurisdiction = toupper(Jurisdiction))

write_csv(allInst2, path = paste0(outPath, 'allInst.csv'))
saveRDS(allInst2, file = paste0(outPath, 'allInst.rds'))
