...
source(paste0(wd, '/WetWeatherLoadingCalcs/RscriptsWet_202324/1WW_dataimport.R'))


stn_RC_202324b_all <- read_csv(paste0(wd, '/WetWeatherLoadingCalcs/Output/stn_RC_202324b_all.csv'))
...
stn_RC_202324b_all <- read_csv(paste0(wd, '/WetWeatherLoadingCalcs/Output/stn_RC_202324b_all.csv')) %>%
  as_tibble() %>%
  select(LSPC_LU_DESC, LSPC_LU_CODE, RC, RC_Group, RC_M, RCandCF, RC_WMAcf_M, EMC_Group, FACILITYID, AREA, station, Rain_Gage_Total, Rain_Event_Total, 'Stormflow Volume -cf', 'Composite Begin', 'Composite End','Sample Type','HSN', 'Fraction', 'Collected Date', 'Stormwater Volume (acre-ft)_RC_Event', 'Stormwater Volume (acre-ft)_RC','Stormwater Volume (acre-ft)_RC_M','Stormwater Volume (acre-ft)_RCandCF', 'Stormwater Volume (acre-ft)_RC_WMAcf_M') %>%
  filter(!is.na(LSPC_LU_DESC)) 

str(stn_RC_202324b_all)
...
stn_load_print_RC <-stn_load %>% 
  select(Station, HSN, 'Sample Type', 'Composite Begin', 'Composite End', Parameter, Fraction, LSPC_LU_DESC,  `Stormflow Volume -cf`,`Stormwater Volume (acre-ft)_RC`, RC, RC_Group, PL_RC) %>% 
  distinct() %>% 
...
stn_load_print_RC_M <-stn_load %>% 
  select(Station,HSN, 'Sample Type', 'Composite Begin', 'Composite End', Parameter, Fraction, LSPC_LU_DESC, `Stormflow Volume -cf`,`Stormwater Volume (acre-ft)_RC_M`, RC_M, RC_Group, PL_RC_M) %>% 
  distinct() %>% 
...
stn_load_print_RC_Event <-stn_load %>% 
  select(Station,HSN, 'Sample Type', 'Composite Begin', 'Composite End', Parameter, Fraction, LSPC_LU_DESC,  `Stormflow Volume -cf`,`Stormwater Volume (acre-ft)_RC_Event`,  RC_Group, PL_RC_Event) %>% 
  distinct() %>% 
...
stn_load_print_RCandCF <-stn_load %>% 
  select(Station,HSN, 'Sample Type', 'Composite Begin', 'Composite End', Parameter, Fraction, LSPC_LU_DESC,`Stormflow Volume -cf`,`Stormwater Volume (acre-ft)_RCandCF`, RCandCF, RC_Group, PL_Load_RCandCF) %>% 
  distinct() %>% 
...
stn_load_print_WMAcf_M <-stn_load %>% 
  select(Station, HSN, 'Sample Type', 'Composite Begin', 'Composite End', Parameter, Fraction, LSPC_LU_DESC,`Stormflow Volume -cf`,`Stormwater Volume (acre-ft)_RC_WMAcf_M`, RC_WMAcf_M, RC_Group, PL_RC_WMAcf_M) %>% 
  distinct() %>% 
