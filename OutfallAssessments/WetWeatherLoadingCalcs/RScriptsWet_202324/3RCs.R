...
landuse_lspc_trib_2024 <- read_csv(paste0(wd, '/WetWeatherLoadingCalcs/Output/landuse_lspc_trib_2024.csv'))


                     # Runoff coefficient 'RC' reality  #43560 cf in an acre-foot
stn_RC_202324 <- EMCRainTot2 %>% 
  mutate(
...
stn_RC_202324 <- stn_RC_202324 %>% 
  ...
  left_join(
    ., landuse_lspc_trib_2024, by=c('Station'= 'FACILITYID', 'RC_Group', 'LSPC_LU_DESC')
  ) %>%
select(-RC) %>%
  mutate (
...
write_csv(stn_RC_202324, paste0(wd, '/WetWeatherLoadingCalcs/Output/stn_RC_202324.csv'))
...
landuse_wcf<-landuse_lspc_trib_2024 %>% 
  right_join(
    .,
    stn_RC_202324,
...
RC_WMAcf_M <- landuse_lspc_trib_2024 %>% 
...
RC_WMAcf_M2 <- landuse_lspc_trib_2024 %>% 
  select(RC_Group, FACILITYID, AREA) %>% 
  left_join(., landuse_wcf2, 
            by=c('RC_Group' = 'RC_Group'))
...
write_csv(stn_RC_202324b, paste0(wd, '/WetWeatherLoadingCalcs/Output/stn_RC_202324b.csv'))
saveRDS(stn_RC_202324b, paste0(wd, '/WetWeatherLoadingCalcs/Output/stn_RC_202324b.rds'))
...
str(landuse_lspc_trib_2024)
str(stn_RC_202324)


landuse_all <-landuse_lspc_trib_2024 %>% 
  right_join(
    .,
    stn_RC_202324,
...
write_csv(stn_RC_202324b_all, paste0(wd, '/WetWeatherLoadingCalcs/Output/stn_RC_202324b_all.csv'))
saveRDS(stn_RC_202324b_all, paste0(wd,'/WetWeatherLoadingCalcs/Output/stn_RC_202324b_all.rds'))
