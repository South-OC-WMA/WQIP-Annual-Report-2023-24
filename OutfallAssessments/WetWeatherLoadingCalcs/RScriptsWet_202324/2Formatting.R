...
values <- list()
values[['ReportStart']]       <- '2023-10-01'
values[['ReportEnd']]         <- '2024-09-30'
...
landuse_jurisdiction_2024 <- read_csv(paste0(wd, '/juris_lu_summary.csv'))


# Combine jurisdiction land use with Geosyntec rc and sbpat emc categories and run-off coefficient  see line 582
landuse_lspc_juris_2024<-distinct(sbpat_landuse_Geosyntec, LSPC_LU_CODE, .keep_all=TRUE) %>%
  select(LSPC_LU_DESC, LSPC_LU_CODE, RC_Group, EMC_Group
  ) %>%
  left_join(., landuse_Geosyntec) %>%
  left_join(
    .,
    landuse_jurisdiction_2024,
    by = c('LSPC_LU_DESC'='lspc_lu_edit')) 
#%>% 
#ungroup()
write_csv(landuse_lspc_juris_2024, paste0(wd, '/WetWeatherLoadingCalcs/Output/landuse_lspc_juris_2024.csv'))

SOCWMA_lu<-landuse_lspc_juris_2024 %>%
  group_by(EMC_Group) %>%
...
landuse_lspc_trib_2024<-distinct(sbpat_landuse_Geosyntec, LSPC_LU_CODE, .keep_all=TRUE) %>%
  select(LSPC_LU_DESC, LSPC_LU_CODE, RC_Group, EMC_Group
  ) %>%
  left_join(., landuse_Geosyntec) %>%
  left_join(
...
#%>% 
#ungroup()
write_csv(landuse_lspc_trib_2024, paste0(wd, '/WetWeatherLoadingCalcs/Output/landuse_lspc_trib_2024.csv'))

landuse_lspc_trib_2024 <- read_csv(paste0(wd, '/WetWeatherLoadingCalcs/Output/landuse_lspc_trib_2024.csv'))

getwd()

MO<- read_csv("MajorOutfalls_2024.csv") %>%
  select('Facility Identifier', Jurisdiction)

MO_LU <- landuse_lspc_trib_2024 %>% 
