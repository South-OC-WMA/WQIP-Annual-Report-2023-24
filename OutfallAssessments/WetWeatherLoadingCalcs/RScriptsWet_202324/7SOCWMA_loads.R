...
landuse_lspc_juris_2024 <- read_csv(paste0(wd, '/WetWeatherLoadingCalcs/Output/landuse_lspc_juris_2024.csv'))
...
trib_load <- SWVolumes %>% 
  ...
  mutate(
    LU_Load = EMC_WMA_LU * Trib_LU_Volume_RC
  ) %>% 
  mutate(
...
write_csv(trib_load, paste0(wd, '/WetWeatherLoadingCalcs/Output/TribLoadsLU_Geosyntec_pre.csv'))
...
juris_loada <-landuse_lspc_juris_2024 %>%
  ...
  mutate(
    Area_Rainfall = 20.2, #average of rainfall event days >0.1" update every year avg from cooperative obs stations
...
write_csv(tribformat2, paste0(wd, '/WetWeatherLoadingCalcs/Output/TribLoadsLU_Geosyntecf.csv'))
...
write_csv(tribformat3, paste0(wd, '/WetWeatherLoadingCalcs/Output/TribLoadsLU_Geosynteccf2.csv'))
...
jurisformat<-     #shows distinct EMC_Group and RC_Group and JURISDICTI
  merge(   
    ...
    (landuse_lspc_juris_2024 %>% ungroup() %>% select(jurisdicti) %>% distinct(jurisdicti)) 
  )  %>%
